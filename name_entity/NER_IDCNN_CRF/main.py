...
home='/disk/chenmingwei/fudan_report/name_entity/NER_IDCNN_CRF/'
flags = tf.app.flags
...
flags.DEFINE_integer("lstm_dim", 256, "Num of hidden units in LSTM, or num of filters in IDCNN")
...
flags.DEFINE_integer("gru_dim", 200, "one layer number of cell")
flags.DEFINE_string("ckpt_path", os.path.join(home,"ckpt_best_medical2"), "Path to save model")
flags.DEFINE_string("ckpt_best_path", "ckpt_0.721", "Path to save model")
...
flags.DEFINE_string("filepath", "/disk/chenmingwei/fudan_report/name_entity/RE_BGRU_2ATT/data/ruijin_round1_train2_20181022", "train file path")
flags.DEFINE_string("test_filepath", "./datasets/ruijin_round1_test_a_20181022", "test file path")
flags.DEFINE_string("test_b_filepath", "./datasets/ruijin_round1_test_b_20181112", "test file path")
flags.DEFINE_string("ner_result", "./result/ner_result_b", "Path for results")
...
def train_new():
    train_sent = change_format(FLAGS.filepath)
    update_tag_scheme(train_sent, FLAGS.tag_schema)#如果用的是BIEO模式标注，则进行更改

    if not os.path.isfile(FLAGS.map_file):
        if FLAGS.pre_emb:
            dico_chars_train = char_mapping(train_sent, FLAGS.lower)[0]
            print(dico_chars_train,"222")
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i in range(FLAGS.max_epoch):
            ...
            plt.pause(0.001)
            # save_model(sess, model, FLAGS.ckpt_path, logger, iteration)
            if best:
...
def test():
    ...
    with tf.Session(config=tf_config) as sess:
        model = create_model(sess, Model, FLAGS.ckpt_best_path, load_word2vec, config, id_to_char, logger)
        ...
        for f1 in files:
            ...
            with codecs.open(os.path.join(FLAGS.test_b_filepath, f1), "r", "utf-8") as f2:
                ...
                f3 = codecs.open(os.path.join(FLAGS.ner_result, f_name + ".ann"), "w", "utf-8")
                i = 0
                for info in line:
                    result = model.evaluate_line(sess, input_from_line(info, char_to_id), id_to_tag)
                    tag = result['entities']
                        ...
                        if char['word'].find("\n") == 0 or char['word'].find(" ") == 0:
                            ...
                            if char['word'].endswith("\n") or char['word'].endswith(" "):
                                ...
                                sent = sent + str(start) + " " + str(char['start'] + leng + j) + ";" + str(
                                    char['start'] + leng + j + 1) + " " + str(char['end'] + leng) + "\t" + word_new.replace("\n", " ")
                            else:
                            ...
                            if char['word'].endswith("\n") or char['word'].endswith(" "):
                                ...
                                j = char['word'].find("\n")
                                sent = sent + str(start) + " " + str(char['start'] + leng + j) + ";" + str(
                                    char['start'] + leng + j + 1) + " " + str(char['end'] + leng) + "\t" + char['word'].replace("\n", " ")
                            else:
