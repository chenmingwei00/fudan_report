...
class Model(object):
    def __init__(self, config):
        ...
        length = tf.reduce_sum(used, reduction_indices=1)   # 计算batch中每个句子的实际长度
        self.lengths = tf.cast(length, tf.int32)
        self.batch_size = tf.shape(self.char_inputs)[0]
        self.num_steps = tf.shape(self.char_inputs)[-1] #句子扩充之后的长度

        # Add model type， bilstm or idcnn
        ...
        self.embedding_dim = self.char_dim + self.seg_dim  #字符的维度增加分词特征的维度
        self.repeat_times = 4
        ...
        if self.model_type == 'bilstm':
            ...
            # logits for tags 每一个数据对不同字符进行类别预测
            ...
            model_outputs1 = tf.nn.dropout(model_outputs, self.dropout)

            # logits for tags
            self.logits = self.project_layer_idcnn(model_outputs1)
        ...
        with tf.variable_scope("optimizer"):
            optimizer = self.config["optimizer"]
            # if self.global_step.eval() > 15000:
            #     optimizer = "sgd"
            if optimizer == "sgd":
