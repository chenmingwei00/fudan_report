...
def update_tag_scheme(sentences, tag_scheme):
    ...
    for i, s in enumerate(sentences):
        tags = [w[-1] for w in s] #每一个字符对应的类别
        # Check that tags are given in the IOB format
        if not iob2(tags): #检查标注是否是BIO的形式
...
def tag_mapping(sentences):
    ...
    dico = create_dico(tags)#创建了类别对应的频数的字典
...
if __name__ == '__main__':
    ...
    flag = main.FLAGS
    arr = load_sentences(flag.train_file, flag.lower, flag.zeros)
